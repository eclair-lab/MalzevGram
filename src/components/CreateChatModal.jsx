...
export default function CreateChatModal({ session, onClose, onChatCreated }) {
  ...
  const handleCreate = async () => {
      ...
      if (myIds.length) {
        ...
        if (existing) {
          const { data: c } = await supabase
            .from('chats').select('*').eq('id', existing.chat_id).single()
    ...
    if (error) { console.error('Ошибка создания чата:', error); setLoading(false); return }
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50" onClick={onClose}>
      <div
        className="bg-[#1a1d27] rounded-2xl w-[360px] p-6 shadow-2xl border border-[#2a2d3a]"
      ...
      >
          ...
          {['dm', 'group'].map(m => (
            <button key={m} onClick={() => { setMode(m); setSelected([]) }}
              className={`flex-1 py-2 rounded-xl text-xs font-medium transition ${
                mode === m ? 'bg-blue-600 text-white' : 'bg-[#0f1117] text-gray-400 hover:text-white'
              }`}>
        ...
        {fetchError && (
          <p className="text-red-400 text-xs mb-2 px-1">Ошибка: {fetchError}</p>
        )}
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-4">
              <p className="text-gray-500 text-xs">
                {users.length === 0 ? 'Загрузка пользователей...' : 'Никого не найдено'}
              </p>
              {users.length === 0 && (
        ...
        {mode === 'group' && selected.length > 0 && (
          <p className="text-xs text-gray-400 mb-3">
            Выбрано: {selected.map(u => u.username).join(', ')}
          </p>
        )}

        <div className="flex gap-2">
          <button onClick={onClose}
            className="flex-1 py-2 rounded-xl bg-[#0f1117] text-gray-400 hover:text-white text-xs transition">
            Отмена
          </button>
